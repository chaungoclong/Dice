...
class Dice
{
    public const CONSTANT = 'Dice::CONSTANT';
    public const GLOBAL = 'Dice::GLOBAL';
    public const INSTANCE = 'Dice::INSTANCE';
    public const CHAIN_CALL = 'Dice::CHAIN_CALL';
    public const SELF = 'Dice::SELF';
    /**
     ...
     * @param array  $rule The container can be fully configured using rules provided by associative arrays.
    ...
    private function addRuleTo(Dice $dice, string $name, array $rule): void
    {
        ...
        $dice->rules[strtolower(ltrim($name, '\\'))] = array_replace_recursive($dice->getRule($name), $rule);
    }

    /**
     * Returns the rule that will be applied to the class $name when calling create()
     *
     * @param string $name name The name of the class to get the rules for
     *
        ...
        foreach ($this->rules as $key => $rule) {   // Find a rule which matches the class described in $name where:
            if (empty($rule['instanceOf'])          // It's not a named instance, the rule is applied to a class name
                ...
                && (!array_key_exists(
                        'inherit',
                        $rule
                    ) || $rule['inherit'] === true))    // And that rule should be inherited to subclasses
            {
                return $rule;
            }
        }
     ...
     * @throws ReflectionException
     */
    private function getClosure(string $name, array $rule): callable
    {
        ...
        } // Get a closure based on the type of object being created: Shared, normal or constructors
        elseif ($params) {
            $closure = static function (array $args, array $share) use ($class, $params) {
            ...
            $closure = static function () use ($class) { // No constructor arguments, just instantiate the class
    ...
    /**
     * Looks for Dice::INSTANCE, Dice::GLOBAL or Dice::CONSTANT array keys in $param and when found returns an object based on the value
     ...
     */
    private function getParams(ReflectionMethod $method, array $rule): callable
    {
            ...
            foreach ($paramInfo as [$class, $param, $sub]) {
                ...
                elseif ($args && $param->getType()) {
                    foreach ($args as $i => $iValue) {
                        if (call_user_func('is_' . $param->getType()->getName(), $iValue)) {
